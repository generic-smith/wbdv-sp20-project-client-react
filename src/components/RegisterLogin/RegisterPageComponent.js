...
class RegisterPageComponent extends React.Component {
  ...
  verifyAndSend = () => {
    ...
    } else if (this.state.user.username === "guest" || this.state.user.username
        === "Guest") {
    ...
    return (
        <div className="body stretch-down">
          <div className="container">
            ...
            {this.state.passNotMatch === 1 && <ErrorMessage
                msg="The two passwords do not match or one of them is empty"/>}
            {/** we locally realized that they tried to register as guest **/}
            {this.state.passNotMatch === 2 && <ErrorMessage
                msg="You cannot register as the Guest user!"/>}
            {/** server tells us no because there already exists a user with that username **/}
            {this.props.registerSuccess === 0 && <ErrorMessage
                msg="Account with that username already exists"/>}
            {/** Registration was successful so bring them to login page **/}
            {this.props.registerSuccess === 1 && this.props.history.push(
                "/login")}
            <h1>Register</h1>
            <form>
              ...
              <div className="form-group row">
                ...
                <div className="col-sm-10">
                  <a href="https://docs.google.com/document/d/1nXB0DihpQNEEqejrVCz-cpkydyO_zMVzanne5VZhgIg/edit?usp=sharing">
                    Privacy Policy
                  </a>
                  <br></br>
                  <b>By clicking the register button below you agree to our privacy policy. Please
                    review it briefly before you register.</b>
                  <button type={"button"}
                          className="btn btn-primary btn-block wbdv-login"
                          id="registerBtn"
                          onClick={this.verifyAndSend}>Register
                  </button>
                </div>
              </div>
