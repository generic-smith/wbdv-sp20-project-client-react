  ...
  componentDidMount() {
    ...
    userService.findUserByUsername(this.props.username).then(response => {
      this.setState({
        ...
        userType: (response.userType !== null) ? response.userType : 'User',
          ...
          <div className={"container"}>
            ...
            <div className={"d-flex"}>
              <button className="btn btn-danger mt-2 mb-2 mr-2"
                      onClick={this.goBack}><i className="fa fa-arrow-left"></i>
              </button>
              <h1><b>Profile</b></h1>
            </div>
            <hr/>
            <div className={"form-group"}>
              ...
              {!this.state.viewOnly && <div
                  className={"profile-password form-inline"}>
                <h5>Password</h5>
                <input type="password"
                       className={"form-control col-5 form-password"}
              ...
              {!this.state.viewOnly && <div
                  className={"profile-confirmed-password form-inline"}>
                <h5>Verify Password</h5>
                <input type={"password"}
                       className={"form-control col-5 form-confirmed"}
                       value={this.state.confirmed}
                       onChange={(e) => this.setState(
                           {confirmed: e.target.value})}/>
                {this.state.password !== '' && this.state.password
                === this.state.confirmed &&
                <div className={"alert alert-success match-alert"}
                     role={"alert"}>
                  Password matched
                </div>}
                {this.state.confirmed !== '' && this.state.password
                !== this.state.confirmed &&
              ...
              <button type="button"
                      className="btn btn-success btn-lg btn-block profile-update"
                      onClick={() => {
              ...
              <button type="button"
                      className="btn btn-danger btn-lg btn-block profile-cancel"
