...
class QueryBar extends React.Component {
  ...
  searchMovies = (title) => {
    fetch(
        `https://api.themoviedb.org/3/search/movie?api_key=8ae10d69b6a533cb7f6179647228aab1&query=${title}`)
        ...
        <div>
          <div>
            ...
            <button className="new-media" onClick={() => {
              this.state.searchType === "search-movies" && this.searchMovies(
                  this.state.searchMovieTitle);
              this.state.searchType !== "search-movies" && alert(
                  "not valid input")
            }}>+
            </button>
